...
struct sssp_result bellman_ford(struct graph *graph, int start) {
    float *dist = calloc(graph->n_nodes, sizeof(int));
    for (int i = 0; i < graph->n_nodes; i++) dist[i] = INFINITY;
    dist[start] = 0;
    int *pred = calloc(graph->n_nodes, sizeof(int));
    for (int i = 0; i < graph->n_nodes; i++) pred[i] = -1;

    for (int i = 0; i < graph->n_nodes; i++)
        for (struct edge *edge = graph->all; edge; edge = edge->next_all)
            relax(dist, pred, edge);
    return (struct sssp_result){start, dist, pred};
}
...
struct sssp_result dijkstra(struct graph *graph, int start) {
    ...
    int *visited = calloc(graph->n_nodes, sizeof(int));

    int *not_visited_heap = calloc(graph->n_nodes, sizeof(int));
    for (int i = 0; i < graph->n_nodes; i++) not_visited_heap[i] = i;
    int *not_visited_node2heap = calloc(graph->n_nodes, sizeof(int));
    for (int i = 0; i < graph->n_nodes; i++) not_visited_node2heap[i] = i;
    int n_not_visited = graph->n_nodes;

    while (n_not_visited) {
        int node = pop_root(not_visited_heap, not_visited_node2heap, dist,
                            n_not_visited--);
        visited[node] = 1;
        for (struct edge *edge = graph->out[node]; edge; edge = edge->next_src) {
            relax(dist, pred, edge);
            if (visited[edge->dst]) continue;
            swim(not_visited_heap, not_visited_node2heap, dist, n_not_visited,
                 not_visited_node2heap[edge->dst]);
        }
    }
    return (struct sssp_result){start, dist, pred};
}
