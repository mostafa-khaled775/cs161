...
struct activity_list *best_schedule(struct activity_list *all) {
    quicksort(all->activities, 0, all->n_activities);

    struct activity_list *schedule = calloc(1, sizeof(*schedule));
    struct activity *last_activity = 0;
    for (int i = 0; i < all->n_activities; i++) {
        struct activity *activity = all->activities[i];
        if (last_activity && activity->start < last_activity->end) continue;
        append_activity(schedule, activity);
